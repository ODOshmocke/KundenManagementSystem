...
if os.path.exists('/Datenschutz.pdf'):
    ...
    pdf = fitz.open('pdfVorlagen/Datenschutz.pdf')
...
#call ervry function
personen_daten('Max Mustermann', '01.01.2000', 'Musterstraße 1, 12345 Musterstadt', 'test', 'test', 'test', 'test')
gegenstand_der_verordnung_einlagen()
gegenstand_der_verordnung_zurichtung()
gegenstand_der_verordnung_os()
sonstiges('test')
werbung_ja()
werbung_nein()
...
# PDF-Datei speichern
if os.path.exists(dateiSpeichern) != True:
    os.mkdir(dateiSpeichern)

try:
    pdf.save(f'{dateiSpeichern}/Datenschutz.pdf')
except:
    pdf.save(f'{dateiSpeichern}/Datenschutz.pdf',  incremental=True, encryption=fitz.PDF_ENCRYPT_KEEP)
pdf.close()

#jedde funkton mit dem jewaliegn funktionsnamen
